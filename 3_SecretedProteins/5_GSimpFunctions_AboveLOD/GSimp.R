...
source('Prediction_funcs.R', local = TRUE)

## Draw n samples from a truncated normal distribution N(mu, std^2|[lo, hi]) ##
rnorm_trunc <- function (n, mu, std, lo=data_raw_log_sc_mdlvec, hi= Inf) {
...
single_impute_iters <- function(x, y, y_miss, y_real=NULL, imp_model='glmnet_pred', lo=-Inf, hi= mdl, iters_each=100, gibbs=c()) {
  y_res <- y
  x <- as.matrix(x)
...
multi_impute <- function(data_miss, iters_each=100, iters_all=20, initial='qrilc', lo=data_raw_log_sc_mdlvec, hi= Inf, 
