---
title: "HBEC Acute vs Repeated Chronic Red Oak Exposure: Secreted Protein Analysis (Core Immune Data Cleaning)"
...
prot_data_raw <- read.xlsx("1_RawData/RawData_SecretedProteins_CoreImmune.xlsx")

# Proteins on the original CoreImmune lsit
full_list <- read.xlsx("2_SampleInfo/nELISA_Targets_CoreImmune.xlsx", sheet = 2) %>%
  row_to_names(1)
...
prot_key[248,6] <- "TGF-beta_1_LAP"
prot_key[249,6] <- "TGF-beta_1_total"
prot_key[1,6] <- "TNFRSF9"
prot_key[91,6] <- "CXCL12_alpha"
prot_key[92,6] <- "CXCL12_beta"
...
prot_data <- prot_data %>% 
  ...
  # Select only columns of interest
  select(c(subgroup:donor_or_rep, treatment, rager_sample_id, TNFRSF9:YWHAZ)) %>%
  
  # Move TNFRSF9 to alphabetical location
  relocate(TNFRSF9, .after = "TNFRSF4")
```

What proteins were in the Core Immune list but not measured?
```{r}
ci_prots <- full_list$UniProt

experiment_prots <- prot_key$Uniprot_ID

missing <- setdiff(ci_prots, experiment_prots)

missing_df <- tibble(UniProt = missing) %>%
  left_join(full_list, by = "UniProt")
```
...
prots_llod_summary <- prot_data %>% 
  group_by(subgroup) %>%
  summarise(across(AGER:YWHAZ, ~ sum(. == "&lt;LLOD"))) %>%
...
prots_ulod_summary <- prot_data %>% 
  group_by(subgroup) %>%
  summarise(across(AGER:YWHAZ, ~ sum(. == "&gt;ULOD"))) %>%
  column_to_rownames("subgroup") %>%
  t() %>% data.frame() %>% 
...
prot_data <- prot_data %>%
  ...
  mutate(across(AGER:YWHAZ, \(x) sub("&lt;LLOD", NA, x))) %>%
  mutate(across(AGER:YWHAZ, \(x) sub("&gt;ULOD", NA, x))) %>%
  mutate(across(AGER:YWHAZ, \(x) as.numeric(x)))
```
...
prot_data_allNA <- prot_data %>%
  summarise(across(AGER:YWHAZ, ~ all(is.na(.)))) %>%
  select_if(~ .)
...
# Write out
write.xlsx(prot_data_filt %>% 
             select(-c(treatment, subgroup:donor_or_rep)) %>% 
             rename("sample_id" = "rager_sample_id"),
           "3_ProcessedData/ProcessedData_CoreImmune_RawData_Formatted_Filtered.xlsx")
```
...
prots_missing_pre <- prot_data_filt %>%
  filter(subgroup == "Pre") %>%
  summarise(across(AKT1:YWHAZ, ~ sum(is.na(.)))) %>%
  t() %>% data.frame() %>% 
  rename("NA_Count" = ".") %>%
  rownames_to_column("Protein")
...
det_hist_byexpo_andday <- ggplot(prot_data_forhist_byexpo_andday, aes(x = value)) +
  ...
  scale_y_continuous(breaks = seq(0, 140, by = 20), limits = c(0, 140), expand = c(0, 0)) +
...
#png("3_OutputFigs/2_DataCleaningCoreImmune/Hist_ProtDet_Pre_CoreImmune.png", width = 10, height = 5, res = 1200, units = "in")
#det_hist_byexpo_andday
#invisible(dev.off())
```
...
# Optionally save figure
#png("3_OutputFigs/2_DataCleaningCoreImmune/Hist_ProtDet_Post_CoreImmune.png", width = 10, height = 5, res = 1200, units = "in")
#det_hist_byexpo_andday
#invisible(dev.off())
```
...
prot_data_filt_pre <- prot_data_filt %>%
  filter(subgroup == "Pre") %>%
  group_by(treatment, timepoint) %>%
  summarise(across(AKT1:YWHAZ, ~ sum(!is.na(.x))))
...
Prepare summary dataframe for detection of each protein:
```{r}
# Create dataframe and join initial detection dataframe
prot_det_summary <- data.frame(Protein = colnames(prot_data %>% select(AGER:YWHAZ))) %>%
...
# Save data
write.xlsx(prot_key_final, "3_ProcessedData/ProcessedData_CoreImmune_ProteinKey.xlsx")
```
...
prots_impgraph_pre <- c("TNFRSF4", "SERPINA1", "CXCL9", "CD163", "MMP8", "LBP", "CCL18", "TNFRSF9", "CXCL11")
...
# Optionally save figure
#pdf(file = "3_OutputFigs/2_DataCleaningCoreImmune/GSimp_Panel_Pre_LLOD_CoreImmune.pdf",
   # width = 7,
   # height = 5)
...
# Manually select proteins to graph
prots_impgraph_post <- c("NFKB2", "MAPK3", "TRAIL", "TSLP", "CCL24", "TGF_R2", "TWEAK", "CCL26", "MUC16")
...
# Optionally save figure
#pdf(file = "3_OutputFigs/2_DataCleaningCoreImmune/GSimp_Panel_Post_LLOD_CoreImmune.pdf",
    #width = 7,
    #height = 5)
...
Prepare dataframes by joining complete data columns to the proteins that were above the ULOD. This also involves removal of some of the complete data that had very low variance. For whatever reason, the above LOD code seems to be more sensitive to this than the below LOD code. The prob = part of the filtering can be experimented with to try to retain as much data as possible while not running into the "nulldev" error. I also had to play around here with removing some of the mediators which also helped prevent the error (a combination of toggling prob and mediators removed). 
```{r}
## PRE
...
prot_data_complete_filt <- prot_data_complete[, which(sds > quantile(sds, prob = 0.9, na.rm = TRUE))] 
...
sds <- apply(prot_data_complete, 2, sd)
prot_data_complete_filt <- prot_data_complete[, which(sds > quantile(sds, prob = .08, na.rm = TRUE))] 
...
# Optionally save figure
#pdf(file = "3_OutputFigs/1_DataCleaningMaxPlex/GSimp_Panel_Pre_ULOD_CoreImmune.pdf",
    #width = 5,
    #height = 3)
#imputation_panel_pre
#invisible(dev.off())
...
# Optionally save figure
#pdf(file = "3_OutputFigs/1_DataCleaningMaxPlex/GSimp_Panel_Post_ULOD_CoreImmune.pdf",
    #width = 3.5,
   # height = 2)
#imputation_panel_post
#invisible(dev.off())
...
```{r}
# Pre data - need to manually add back in those with -, which got changed to periods during imputation
prot_data_pre_final_imp <- prot_data_imp_pre_aboveLOD %>%
  select(c(GDF.15, any_of(prots_aboveULOD_pre))) %>%
  rownames_to_column("rager_sample_id") %>%
...
prot_data_post_final_imp <- prot_data_imp_post_aboveLOD %>%
  select(c(any_of(prots_aboveULOD_post))) %>%
  rownames_to_column("rager_sample_id") %>%
  left_join(prot_data_imp_post %>% rownames_to_column("rager_sample_id"), by = "rager_sample_id") %>%
  rename("sample_id" = "rager_sample_id")

# Write out 
write.xlsx(prot_data_pre_final_imp, "3_ProcessedData/ProcessedData_CoreImmune_Imputed_Pre.xlsx")
write.xlsx(prot_data_post_final_imp, "3_ProcessedData/ProcessedData_CoreImmune_Imputed_Post.xlsx")
```
