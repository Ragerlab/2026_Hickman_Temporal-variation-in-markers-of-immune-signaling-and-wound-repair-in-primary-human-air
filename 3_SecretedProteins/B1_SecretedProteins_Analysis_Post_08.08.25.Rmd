---
title: "HBEC Acute vs Repeated Red Oak Exposure: Secreted Protein Analysis (Between Groups and Time Points) - Post Exposure Samples"
...
We are focusing only on the post-exposure data because there is more consistency in the amount of time that the cells had been incubating with the media and because this also reflects the same time that the samples for RNAseq, with which we want to compare the results, were collected. 

Import data:
```{r} 
# MaxPlex
max <- read.xlsx("3_ProcessedData/ProcessedData_MaxPlex_Imputed_Post.xlsx")

# CoreImmune
ci <- read.xlsx("3_ProcessedData/ProcessedData_CoreImmune_Imputed_Post.xlsx")
```
...
post <- left_join(ci, max %>% select(c(sample_id, all_of(prots_unique_maxplex))), by = "sample_id")
```
...
hist <- ggplot(post %>% pivot_longer(!sample_id, names_to = "protein", values_to = "value"), aes(value)) +
...
png("6_Figures/1_Post/Hist_Panel_Post.png", width = 12, height = 10, units = "in", res = 300)
hist
invisible(dev.off())

# Make figure panel of Q-Q plots
qq <- ggqqplot(post %>% pivot_longer(!sample_id, names_to = "protein", values_to = "value"), 
         x = "value", facet.by = "protein", ggtheme = theme_bw(), scales = "free")

png("6_Figures/1_Post/QQ_Panel_Post.png", width = 30, height = 30, units = "in", res = 300)
qq
invisible(dev.off())
```
...
png("6_Figures/1_Post/Hist_Panel_Log2_Post.png", width = 12, height = 10, units = "in", res = 300)
hist_log2
invisible(dev.off())

# Make figure panel of Q-Q plots
qq_log2 <- ggqqplot(post %>% 
           pivot_longer(!sample_id, names_to = "protein", values_to = "value") %>%
           mutate(value = log2(value + 1)), 
         x = "value", facet.by = "protein", ggtheme = theme_bw(), scales = "free")

png("6_Figures/1_Post/QQ_Panel_Log2_Post.png", width = 30, height = 30, units = "in", res = 300)
qq_log2
invisible(dev.off())
```
...
outliers_6 <- outlier_detection(pca_post, 6)
outliers_5 <- outlier_detection(pca_post, 5)
outliers_4 <- outlier_detection(pca_post, 4)
outliers_3 <- outlier_detection(pca_post, 3)
...
pca_plot_post <- fviz_pca_ind(pca_post, 
             label = "none",
             pointsize = 3) +
  labs(title = "PCA: Post Samples") +
...
pca_plot_post_expo <- fviz_pca_ind(pca_post, 
             label = "none",
             pointsize = 3, 
             habillage = post_key$exposure, 
             mean.point = FALSE, 
             addEllipses = TRUE) +
  labs(title = "PCA: By Exposure (Post)") +
...
# Visualize PCA - Day
pca_plot_post_day <- fviz_pca_ind(pca_post, 
             label = "none",
             pointsize = 3, 
             habillage = post_key$timepoint, 
             mean.point = FALSE,
             addEllipses = TRUE) +
  labs(title = "PCA: By Day (Post)") +
...
pca_patch_post <- pca_plot_post_expo + pca_plot_post_donor + pca_plot_post_day

png("6_Figures/1_Post/PCA_Panel_ByVariable_Post.png", width = 15, height = 4, units = "in", res = 300)
pca_patch_post
invisible(dev.off())

# Save off dataframes for interindividual analysis
save(file = '3_ProcessedData/post_key.Rdata', post_key)
save(file = '3_ProcessedData/post_log2.Rdata', post_log2)
```
...
# Clean summary stats
summary_full_post <- ttest_res_post %>%
  ...
  left_join(summary_stats_post, by = "day_protein") %>%
...
prots_all <- summary_full_post %>% pull("Protein") %>% unique()
...
prot_description_key_coreimmune <- data.frame(Protein_Column_Name = prots_all) %>%
  mutate(Protein_Column_Name = gsub("\\.", "-", Protein_Column_Name)) %>%
  mutate(Protein_Column_Name = str_remove(Protein_Column_Name, "-$")) %>%
  mutate(Protein_Column_Name = str_replace(Protein_Column_Name, "\\s+$", "")) %>%
  mutate(Protein_Column_Name = str_replace(Protein_Column_Name, "^([^\\-]*\\-[^\\-]*)\\-", "\\1.")) %>%
  left_join(prot_key_coreimmune %>% 
              select(c(Uniprot_ID, Protein_Name, Protein_Column_Name)) %>%
              mutate(Protein_Column_Name = str_replace(Protein_Column_Name, "\\s+$", "")) %>%
              mutate(Protein_Column_Name = str_replace(Protein_Column_Name, " ", "\\.")), 
            by = "Protein_Column_Name") %>% 
  na.omit()
...
prot_description_key_maxplex <- data.frame(Protein_Column_Name = prots_all) %>%
  ...
  left_join(prot_key_maxplex %>%
              select(c(Uniprot_ID, Protein_Name, Protein_Column_Name)) %>%
              mutate(Protein_Column_Name = str_replace(Protein_Column_Name, "\\s+$", "")), by = "Protein_Column_Name")
...
summary_full_post_final <- summary_full_post %>%
...
write.xlsx(summary_full_post_final, "7_Tables/SummaryStats_Post.xlsx")
```
...
# Define color scheme
group_colors <- c("Day 1" = "#f3e55d","Day 3" = "#fc9f07","Day 5" = "#e8602d", 
...
upset_plot_incr <- upset(
  ...
  # Groups to plot
  c("Day 12", "Day 10", "Day 8", "Day 5", "Day 3", "Day 1"),
  ...
  ,
    queries=list(
        upset_query(set = 'Day 1', fill='#f3e55d'),
...
png("6_Figures/1_Post/Upset_Post_Increased.png", width = 7, height = 5, units = "in", res = 1200)
upset_plot_incr
invisible(dev.off())
```
...
png("6_Figures/1_Post/Upset_Post_Decreased.png", width = 7, height = 5, units = "in", res = 1200)
upset_plot_decr
invisible(dev.off())
```
...
ttest_sig_prots <- ttest_res_post %>%
  filter(p < 0.05) %>%
  pull("protein")
...
fc_hm_anno <- avg_foldchange %>%
  ...
  filter(protein %in% all_of(ttest_sig_prots)) %>%
  mutate(protein = str_remove_all(protein, "[_.]")) %>%
  left_join(ttest_res_post %>% unite(day_protein, day, protein, sep = " ") %>% select(c(day_protein, p)), by = "day_protein") %>%
...
png("6_Figures/1_Post/Heatmap_Sig_Post.png", width = 5.5, height = 9, units = "in", res = 1200)
hm
invisible(dev.off())
```
...
counts_post_d12 <- counts_raw_filtered %>% 
  pivot_longer(-c(gene, gene_cleaned, uniprot_gn_id, colname_clean), names_to = "sample", values_to = "raw_counts_RNAseq") %>%
  filter(!str_detect(sample, "Ac")) %>%
  mutate(sample_matching = gsub("Ch", "Day12", sample)) %>%
  unite(sample_colname, sample_matching, colname_clean, sep = " ", remove = FALSE)
...
# Combine data
corrprep_d1 <- counts_post_d1 %>%
  left_join(prot_d1, by = "sample_colname")

corrprep_key_d1 <- corrprep_d1 %>%
  select(c(gene, gene_cleaned, colname_clean))

corrprep_d12 <- counts_post_d12 %>%
  left_join(prot_d12, by = "sample_colname")

corrprep_key_d12 <- corrprep_d12 %>%
  select(c(gene, gene_cleaned, colname_clean))
```
...
nrow(corr_d1 %>% filter(p < 0.05))
nrow(corr_d1 %>% filter(abs(cor) >= 0.6))
nrow(corr_d1 %>% filter(abs(cor) >= 0.6 & p < 0.05))

corr_d12 <- corrprep_d12 %>%
  group_by(gene) %>%
  cor_test(raw_counts_RNAseq, log2_prot_expr, method = "spearman")

nrow(corr_d12 %>% filter(p < 0.05))
nrow(corr_d12 %>% filter(abs(cor) >= 0.6))
nrow(corr_d12 %>% filter(abs(cor) >= 0.6 & p < 0.05))
```

Prepare dataframe for running correlation:
```{r warning = FALSE}
# Read in data
counts_norm <- read.xlsx("2_SampleInfo/ProcessedData_BulkRNASeq_DESeq2NormalizedCounts.xlsx")

# Filter count data to only genes of interest
counts_norm_filtered <- counts_raw %>% 
  separate(gene, into = c("gene_cleaned", NA), sep = "_", remove = FALSE) %>%
  filter(gene_cleaned %in% all_of(gene_synonyms$gene_synonym)) %>%
  left_join(gene_synonyms %>% select(c(gene_synonym, uniprot_gn_id)), join_by("gene_cleaned" == "gene_synonym")) %>%
  distinct() %>%
...
# Filter count data to separate pre and post data, and to pivot data longer
counts_post_d1_norm <- counts_norm_filtered %>% 
  pivot_longer(-c(gene, gene_cleaned, uniprot_gn_id, colname_clean), names_to = "sample", values_to = "raw_counts_RNAseq") %>%
  filter(!str_detect(sample, "Ch")) %>%
  mutate(sample_matching = gsub("Ac", "Day1", sample)) %>%
  unite(sample_colname, sample_matching, colname_clean, sep = " ", remove = FALSE)

counts_post_d12_norm <- counts_norm_filtered %>% 
  pivot_longer(-c(gene, gene_cleaned, uniprot_gn_id, colname_clean), names_to = "sample", values_to = "raw_counts_RNAseq") %>%
  filter(!str_detect(sample, "Ac")) %>%
  mutate(sample_matching = gsub("Ch", "Day12", sample)) %>%
  unite(sample_colname, sample_matching, colname_clean, sep = " ", remove = FALSE)
...
# Combine data
corrprep_d1_norm <- counts_post_d1_norm %>%
  left_join(prot_d1, by = "sample_colname")

corrprep_key_d1_norm <- corrprep_d1_norm %>%
  select(c(gene, gene_cleaned, colname_clean))

corrprep_d12_norm <- counts_post_d12_norm %>%
  left_join(prot_d12, by = "sample_colname")

corrprep_key_d12_norm <- corrprep_d12_norm %>%
  select(c(gene, gene_cleaned, colname_clean))
```

How many proteins were matched?
```{r}
nrow(counts_norm_filtered %>% filter(!duplicated(gene_cleaned)))
```


Run correlations
```{r}
corr_d1_norm <- corrprep_d1_norm %>%
  group_by(gene) %>%
  cor_test(raw_counts_RNAseq, log2_prot_expr, method = "spearman")

nrow(corr_d1_norm %>% filter(p < 0.05))
nrow(corr_d1_norm %>% filter(abs(cor) >= 0.6))
nrow(corr_d1_norm %>% filter(abs(cor) >= 0.6 & p < 0.05))

corr_d12_norm <- corrprep_d12 %>%
  group_by(gene) %>%
  cor_test(raw_counts_RNAseq, log2_prot_expr, method = "spearman")

nrow(corr_d12_norm %>% filter(p < 0.05))
nrow(corr_d12_norm %>% filter(abs(cor) >= 0.6))
nrow(corr_d12_norm %>% filter(abs(cor) >= 0.6 & p < 0.05))
```

What was the overlap in correlated proteins between d1 and d12 - is this consistent? 
```{r}
corr_prots_d1 <- corr_d1_norm %>% filter(abs(cor) >= 0.6 & p < 0.05) %>% pull("gene")
corr_prots_d12 <- corr_d12_norm %>% filter(abs(cor) >= 0.6 & p < 0.05) %>% pull("gene")

intersect(corr_prots_d1, corr_prots_d12)

corr_prots_d1 <- corr_d1_norm %>% filter(abs(cor) >= 0.6) %>% pull("gene")
corr_prots_d12 <- corr_d12_norm %>% filter(abs(cor) >= 0.6) %>% pull("gene")
...
deseq_re <- read.xlsx("2_SampleInfo/DESeq2_Results_Repeated.xlsx")
...
ttest_sig_d1 <- ttest_res_post_organized %>%
  filter(day == "Day1") %>%
  rename("colname_clean" = "protein", "log2FC_prot" = "log2FC") %>%
  select(c(colname_clean, log2FC_prot))

ttest_sig_d12 <- ttest_res_post_organized %>%
  filter(day == "Day12") %>%
  rename("colname_clean" = "protein", "log2FC_prot" = "log2FC") %>%
  select(c(colname_clean, log2FC_prot))

# Then pull RNAseq results
deseq_ac_overlap_d1 <- deseq_ac_overlap %>%
  left_join(corrprep_key_d1, by = "gene") %>%
  distinct() %>%
  filter(colname_clean %in% all_of(ttest_sig_d1$colname_clean)) %>%
  left_join(ttest_sig_d1, by = "colname_clean")

count(deseq_ac_overlap_d1 %>% filter(p_deseq < 0.05))
count(deseq_ac_overlap_d1 %>% filter(padj_deseq < 0.05))

deseq_ac_overlap_d12 <- deseq_ac_overlap %>%
  left_join(corrprep_key_d12, by = "gene") %>%
  distinct() %>%
  filter(colname_clean %in% all_of(ttest_sig_d12$colname_clean)) %>%
  left_join(ttest_sig_d12, by = "colname_clean")

count(deseq_ac_overlap_d12 %>% filter(p_deseq < 0.05))
count(deseq_ac_overlap_d12 %>% filter(padj_deseq < 0.05))
```
...
```{r}
# Prepare data
corr_d1_LIF <- corrprep_d1 %>%
  filter(colname_clean == "LIF") %>%
  separate(sample, into = c("Donor", "Exposure", NA), sep = "_") %>%
  mutate(Exposure = recode(Exposure, "CT" = "Vehicle", "RO" = "Red Oak")) %>% 
  mutate(Exposure = factor(Exposure, levels = c("Vehicle", "Red Oak")))

# Plot
LIF_corrplot <- ggplot(corr_d1_LIF, aes(x = raw_counts_RNAseq, y = log2_prot_expr)) +
  geom_point(aes(color = Exposure, fill = Exposure, shape = Exposure), size = 4) + 
  scale_color_manual(name = "Exposure", values = c("steelblue2", "gold1")) +
  scale_fill_manual(name = "Exposure", values = c("steelblue2", "gold1")) +
  scale_shape_manual(name = "Exposure", values = c(15, 16)) +
  labs(x = "RNAseq Counts", y = bquote(~Log[2]~'Protein Concentration'), title = "Day 1 LIF") +
...
png("6_Figures/1_Post/Corrplot_LIF_Day1.png", width = 4, height = 3, units = "in", res = 1200)
LIF_corrplot
invisible(dev.off())
...
```{r}
# Prepare data
corr_d1_BMP3 <- corrprep_d1 %>%
  filter(colname_clean == "BMP3") %>%
  separate(sample, into = c("Donor", "Exposure", NA), sep = "_") %>%
  mutate(Exposure = recode(Exposure, "CT" = "Vehicle", "RO" = "Red Oak")) %>% 
  mutate(Exposure = factor(Exposure, levels = c("Vehicle", "Red Oak")))

# Plot
BMP3_corrplot <- ggplot(corr_d1_BMP3, aes(x = raw_counts_RNAseq, y = log2_prot_expr)) +
  geom_point(aes(color = Exposure, fill = Exposure, shape = Exposure), size = 4) + 
  scale_color_manual(name = "Exposure", values = c("steelblue2", "gold1")) +
  scale_fill_manual(name = "Exposure", values = c("steelblue2", "gold1")) +
  scale_shape_manual(name = "Exposure", values = c(15, 16)) +
  labs(x = "RNAseq Counts", y = bquote(~Log[2]~'Protein Concentration'), title = "Day 1 BMP3") +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        plot.title = element_text(face = "bold", hjust = 0.5),
        legend.position = "right",
...
png("6_Figures/1_Post/Correplot_BMP3_Day1.png", width = 4, height = 3, units = "in", res = 1200)
BMP3_corrplot
invisible(dev.off())
